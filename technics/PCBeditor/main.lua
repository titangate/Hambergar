...
pulse = love.graphics.newImage('pulse.png')
function getPulse()
...
function Chip:draw()
	...
	for k,v in pairs(self.wire) do
		...
		--		if self.activatetime then
		--			local x,y = getInterpoint(v2,self.activatetime)
		--			love.graphics.circle('fill',x*scale,y*scale,5)
		--		end
			end
		end
	end
	...
	love.graphics.print(self.name,self.x*scale,self.y*scale)
end
