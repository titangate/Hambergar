...
function Chip:draw()
	...
	if self.activetime then
		for k,v in self.running do
			local c = v[math.ceil(self.activetime/0.5)]
			if c then
				love.graphics.circle('fill',c[1],c[2],16)
			end
		end
	end
end
...
function Chip:connect(b,path)
	self.wire[b] = self.wire[b] or {length = 0}
	self.wire[b].length = math.max(self.wire[b].length,#path)
 	table.insert(self.wire[b],path)
end

function Chip:activate(b)
	if self.wire[b] then
		self.running = self.wire[b]
		
	end
	self.activatetime = self.wire[b].length*0.5
end
