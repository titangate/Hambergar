...
function goo.object:setAvailable( bool)
	self.available = bool
end
...
function goo:update( dt, object )
	...
	for k,child in pairs(object.children) do
		if child.visible and child.available then self:update(dt,child) end
	end
end
...
function goo:keypressed( key, unicode, object )
	local object = object or self.BASEOBJECT
	local ret = false
	if object.visible and object.available then ret = object:keypressed(key, unicode) 
	for i,child in ipairs(object.children) do
...
function goo:keyreleased( key, unicode, object )
	local object = object or self.BASEOBJECT
	if object.visible and object.available then ret = object:keyreleased(key, unicode) 
		for i,child in ipairs(object.children) do
...
function goo:mousereleased( x, y, button )
	local object = self.BASEOBJECT.mousehover
	if object.visible and object.available then object:mousereleased( x, y, button ) end
end
...
function goo.object:direct(highlight,direction,filter)
	local xbase,ybase = highlight.x,highlight.y
	local objdict = {}
	local distarray = {}
	local x1,y1 = unpack(direction)
	for i,child in ipairs(self.children) do
		if filter(child) and child~=highlight then
			local x2,y2 = child.x-highlight.x,child.y-highlight.y
