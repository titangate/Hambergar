...
local extractMin = function(t,condition)
	if #t == 0 then
		return
	end
	if #t == 1 then
...
local distanceBetween = function(a,b)
...
function Lighter:setGlobalLightsource(s)
	self.global = s
end
...
function Lighter:update(dt)
	...
	for v,_ in pairs(self.obj) do
		if self.global and v:isKindOf(Filter) then
			local b = LightBeam(self.global.x,self.global.y,{0,0,0},math.atan2(v.y-self.global.y,v.x-self.global.x),self)
			b.parent = self
...
function LightBeam:initialize(x,y,color,direction,source,prevbeam)
	self.x,self.y = x,y
	self.color = color
	self.direction = direction
	self.length = 99999
	self.ignores = {[source]=true}
	if prevbeam then
		for v,_ in pairs(prevbeam.ignores) do
			self.ignores[v] = true
		end
	end
end
...
function LightBeam:process()
	...
	for v,_ in pairs(self.parent.obj) do
		if v~=self and not self.ignores[v] then
			local s = true
...
function LightBeam:draw()
	...
	love.graphics.draw(img.whiteray,self.x,self.y,self.direction,self.length,1,0,17.5)
...
function Filter:casted(beam)
	...
	local l = LightBeam(self.x,self.y,newc,beam.direction,self,beam)
	l.parent = self.parent
	self.parent:addBeam(l)
	return true
end
...
function Mirror:casted(beam)
	...
	local l = LightBeam(self.x,self.y,beam.color,dir,self,beam)
	l.parent = self.parent
	self.parent:addBeam(l)
	return true
end
...
function Portal:casted(beam)
	if self.portal then
		local l = LightBeam(self.portal.x,self.portal.y,beam.color,beam.direction+self.direction+self.portal.direction,self.portal,beam)
		l.parent = self.parent
		self.parent:addBeam(l)
		return true
	else
