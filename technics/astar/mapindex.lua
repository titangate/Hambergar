...
local coroutine, pairs, next, select, setmetatable, deepcopy, shallowcopy, table, unpack =
	  coroutine, pairs, next, select, setmetatable, deepcopy, shallowcopy, table, unpack
...
local function yield_keys(t,key)
	...
	for k,t in pairs(t) do
		if k == E then
			coroutine.yield(tuple(unpack(key)))
		else
			nk = shallowcopy(key)
			table.insert(nk,k)
			yield_keys(t,nk)
