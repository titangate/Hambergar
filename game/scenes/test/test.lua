...
function TestMap:initialize()
	...
	self.flows = {}
	self.background = TestMapbackground
	self.emitrate = 1
	self.emittime = 1
	self.birthtime = 0
end
...
requireImage("assets/gridfilter.png",'TestMapfilter')
function TestMap:draw()
	...
	if self.birthtime > 0 then
		...
		--print (love.graphics.getHeight()*(3-self.birthtime)/3+TestMapfilter:getHeight())
		--print (600*(1-self.birthtime)+TestMapfilter:getHeight()*2,self.birthtime)
		map.camera:revert()
		love.graphics.draw(img.TestMapfilter,0,600*(1-self.birthtime)-TestMapfilter:getHeight(),0,25,1,0,0)
		map.camera:apply()
...
function TestMap:opening_load()
	local lawrence = Swift:new(10,10,32,10)
...
function TestMap:loadCheckpoint(checkpoint)
end
